...
flowchart TD
    ...
    F --> G["建立团队协作记录"]
    G --> H["创建真实 subagent 编制"]
    H --> H1{"subagent 是否可用?"}
    H1 -- "否" --> H2["记录 SUBAGENT-BLOCKER"]
    H2 --> Z2["阻断，不生产"]
    H1 -- "是" --> I["能力域 subagent 并行初审"]
    ...
    P -- "否" --> J
    P -- "是" --> P1["阶段B：正式生产平台Skill二次调用/复核"]
    P1 --> Q["执行层生成真实产物"]

    Q --> R["工程烟测：交互/截图非空/溢出；不得替代页数充分性或审美QA"]
    R --> S["实现后一级复核"]
    S --> S1["同一批 subagent 检查真实文件、截图、validation"]
    S1 --> T{"任一副主编退回?"}
...
| Style Frame探索期 | 在用户明确批准后生成1-3页局部视觉方向证据，供恢复生产决策 | 命名Vx、扩展全稿、跑冻结QA、称候选交付 |
| 修改/回退闭环中 | 问题归因、副主编审核、执行层修订、同一副主编复审、全部适用副主编一致审核 | 主Agent直接改完并宣布通过；只由打回副主编同意就继续 |
