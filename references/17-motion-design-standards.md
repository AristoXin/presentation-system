...
| interaction state | idle / active / revealed / completed / reversed / skipped / export 等HTML状态 |
| input contract | 键盘、鼠标、触摸、滚轮、目录跳转、讲者模式分别如何触发或禁止触发 |
| focus/accessibility | 焦点是否移动、aria状态是否更新、屏幕阅读器和键盘用户是否能理解 |
| cancel/throttle | 连点、连按、快速滚轮、返回上一页时如何节流、取消或恢复 |
