...
- HTML状态机：每页和每个分步组件必须有可追踪的状态，而不是把PPT点击动画搬进网页。
...
- 不得把交互实现降级为“点击一次出现下一个元素”。每个触发必须有状态、焦点、节流、回退、目录跳转和export落稳策略。
...
| 目录 | 可从缩略图或章节列表跳转 |
| 讲者提示 | 当前页备注、计时和下一页标题 |
| 状态机 | 每页记录`idle/reveal(n)/completed/reverse/export`等状态；快速翻页、目录跳转、回退和导出都必须落到确定状态 |
| 焦点与可访问性 | 触发分步、打开目录、切换讲者提示时必须维护键盘焦点、`aria-expanded`/`aria-current`等必要状态 |
| 输入节流 | 键盘连按、滚轮连滚、触摸滑动、按钮连点必须节流，避免越级、跳步、半动画或状态错乱 |
| 控制栏 | 默认隐藏、透明或极弱化，靠近底部或触控唤起，不遮挡关键内容；底部操作栏按钮必须采用“中文动作词 + 图标”的组合，不得只显示符号、英文缩写或问号 |

## HTML不是PPT模拟

HTML演示是一个浏览器中的交互应用。设计和实现必须把它当作HTML文件处理：DOM状态、CSS动画、JS事件、焦点管理、触摸/滚轮输入、目录跳转、URL模式、导出截图和打印模式共同决定体验。

| 误区 | 正确做法 |
| :--- | :--- |
| 把每页当PPT页，点击后出现下一个图标/文字 | 把每页定义为状态机，明确首态、每步态、完成态、回退态和导出态 |
| 只为鼠标点击设计 | 同时设计键盘、滚轮、触摸、目录跳转和讲者模式 |
| 用CSS类随手控制显示隐藏 | 由动效脚本和交互合同定义状态，JS只实现合同 |
| 动画完成前允许继续乱触发 | 对输入节流、取消、跳过、落稳做明确规则 |
| 截图时碰运气等待动画结束 | export模式强制隐藏chrome并禁用或落稳动画 |

没有《HTML状态机与交互合同》时，不得进入正式HTML实现；已有实现也只能算工程草稿，不得进入QA冻结。
...
12. 若有分步或互动，核对《HTML状态机与交互合同》：键盘、鼠标、触摸、滚轮、目录跳转、回退和export是否都能进入预期状态。
...
- 截图工具不得直接改`.active`、`.visible`或内部计数器绕过真实状态机；若必须使用测试接口，接口必须模拟正式导航/触发合同并记录当前状态。
