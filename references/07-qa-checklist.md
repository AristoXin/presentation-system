...
- HTML/CSS是否完成`cssToxicityCheck`，并确认玻璃拟态、复杂渐变、大圆角、弥散阴影、无语义多色和无说明默认字体没有违规。
...
- HTML演示是否有《HTML状态机与交互合同》，而不是把PPT点击动画搬进网页。
- 分步页面是否有首态/后续态矩阵，实际HTML初始态和每次触发是否与矩阵一致。
- 分步或互动是否覆盖键盘、鼠标、滚轮、触摸、目录跳转、回退、重播、presenter/audience/export模式。
- 页间转场和同页分步是否具备设计过的运动语法，而不是统一淡入/上移或突兀硬切。
- 是否通过Style Frame审美确认。
- 是否附带《参考经验转译卡》，并说明当前设计如何结合用户资料，而不是照搬参考图。
- 若采用正向参考图，是否附带《正向参考图深拆与经验总结表》，并逐项拆解配色原因、文字协同、字体系统、版面规划、跨页切换、替代方案、动效方案、交互/HTML状态机和迁移边界。
- 若参考图或目标产物包含多页PPT，是否完成《多页视觉系统拆解表》：逐页角色、跨页视觉语法、信息层级、真实视觉锚点、组件语义、动效/静止策略和交付路线。
...
- HTML交互是否有输入节流、焦点管理、aria状态、快速导航和export落稳策略。
- 默认打开时控制条是否隐藏或弱化。
- 鼠标靠近底部时控制条是否平滑出现。
- 鼠标离开后控制条是否自动隐藏。
...
- UI是否避免“方块配色填字式”表层设计：页面结构是否来自内容关系和部门协同，而不是只把文字塞入色块、卡片或图标矩阵。

QA只验收带版本号的冻结稿，不验收临时稿。
未冻结稿若提交用户，只能作为候选方向验收，不得作为正式交付验收；但缺少真实subagent证据或工作平台证据时，不得提交为样张、工作稿或候选方向，只能提交流程阻断说明、失败审计或文字Page Spec。
未通过审美验收的页面不得进入QA终审。
...
QA也必须先检查部门会议、初始备忘录、关键页试生产和工作平台证据。真实产物必须晚于正式全稿制作工单和正式生产工作平台启用，且有《部门会议记录》《交付文件初始备忘录》《关键页试生产记录》《工作平台编制表》《工作平台交付记录》《文件所有权映射》和交回副主编复核记录。缺少任一关键流程证据时，QA立即判定`PROCESS_BLOCKED_NO_WORK_PLATFORM`或相应流程阻断；不得截图、运行Playwright、生成validation、写工程烟测通过或进入人工审美QA。
...
- 是否每个采用的正向参考都已被拆成跨页设计方法，而不是只复用配色、贴纸、斜切块、网格底纹或表层热闹感？
- 是否每个正向参考都回答了“为什么这样配色、文字如何配合、字体/字号/字重/样式为何成立、版面如何规划、页面如何切换、如果我来做会考虑什么方案、如何设计动效与交互”？
...
| HTML状态机复核 | 检查`idle/reveal(n)/completed/reverse/export`等状态及键盘/鼠标/触摸/滚轮/目录跳转 | 把HTML当PPT点击序列，只实现下一项出现 |
...
| 正向参考深拆核验 | 检查《正向参考图深拆与经验总结表》是否覆盖配色、文字、字体、版面、跨页切换、替代方案、动效和交互 | 只写好看、配色鲜明、版式活泼，未形成可迁移设计方法 |
| 多页系统核验 | 对照《多页视觉系统拆解表》检查页面族谱、跨页语法、真实锚点和动效策略 | 把多页参考当单张海报；只复用色块、卡片、贴纸或网格 |
...
11c. 检查HTML状态机硬伤：若互动只靠点击显示下一个图标、表格或文字，缺少状态机、输入节流、焦点/aria、回退/目录跳转和export落稳策略，标记为Interaction退回，不得以“能点出来”通过。
...
- presenter模式和audience模式是否区分明确？
- export模式是否能输出无控件画面？
- 分步触发、回退、目录跳转和快速翻页是否都能保持正确状态？
- 焦点、aria状态和键盘操作是否与当前HTML状态一致？
...
0. QA第0步流程阻断检查：在确认版本、截图、运行Playwright、导出、生成`validation.json`或写工程烟测结论之前，先检查《用户最新要求锁定表》、Subagent Evidence Lock、部门会议、交付文件初始备忘录、关键页试生产和工作平台证据。必须有真实subagent id、真实工具/线程证据、员工中文名称、同一副主编实现后复审记录、部门会议记录、初始备忘录、关键页试生产记录、工作平台编制表、工作平台交付记录、文件所有权映射和`PROCESS_READY_WORK_PLATFORM`结论。若HTML/前端实现适用，QA还必须检查HTML代码员工“蔻豆码”或等价职责中文名是否出现在部门会议、工作平台编制和交付记录中；缺失且无不适用签署时，判定流程阻断。缺少真实subagent证据时，立即输出`PROCESS_BLOCKED_NO_SUBAGENT`；缺少部门会议/初始备忘录/关键页试生产/工作平台证据时，立即输出`PROCESS_BLOCKED_NO_WORK_PLATFORM`或相应流程阻断；不得继续截图、像素、键盘、控件、导出、validation或工程烟测。
0a. QA第0a步CSS毒性静态检查：HTML/CSS产物必须对照`11-aesthetic-anti-ai.md`的`CSS 毒性静态检查`逐项记录`cssToxicityCheck`。任一项`failed`时，停止审美QA并退回Style Frame或进入`FAILED_VISUAL_FREEZE`；例外必须有品牌审美签署和用户/品牌/参考证据来源。
1. 确认版本号和冻结稿路径。
2. 确认生产前卡片齐全：资料盘点、交付定义、部门会议记录、交付文件初始备忘录、内容差距、参考经验转译、UI方向、Style Frame确认。
3. 确认Agent编制表、关键页试生产记录、工作平台编制表、工作平台交付记录、文件所有权映射、副主编一级复核记录、副主编复审循环记录、副总编一致性审核和总编二级复核记录已完成。
4. 逐页截图或导出全页图。
...
无subagent/无工作平台阻断规则：工程烟测不是QA的前置补救。无真实subagent证据、无工作平台编制和交付记录，或无用户明确降级批准时，QA不得生成`validation.json`，不得写“工程烟测通过”，不得创建新的截图证据。只能生成《流程阻断记录》，列明缺失的subagent、缺失轮次、缺失的工作平台员工或文件所有权、是否有用户降级批准和下一步恢复条件。
...
| 正向参考深拆 | 若采用正向参考，是否有配色逻辑、文字协同、字体系统、版面规划、跨页切换、替代方案、动效方案、交互/HTML状态机和迁移边界 |
...
| 工作平台证据 | 工作平台批次、抽调员工Agent/专员、文件所有权映射、交付记录、交回副主编路径、是否存在越权改标准；HTML/前端适用时是否包含“蔻豆码”或等价职责中文名 |
...
| `cssToxicityCheck` | backdropFilter、gradientLayerCount、maxBorderRadius、boxShadowPolicy、hueCount、fontStackPolicy、capsuleOrCardMatrixRisk，每项为 passed/failed/exception_signed |
...
| `workPlatformEvidence` | platformBatchId、activatedAt、activationPrerequisites、staffByDepartment、fileOwnershipMap、workOrders、deliveryRecords、handoffToReviewers、readyStatus、blockers |
