...
| 用户要求完整检查/全面更新本Skill，或要求复盘过去总结/所有evidence是否已按新规则整理 | `SKILL.md` + `AGENTS.md` + `agents/openai.yaml` + 全部`references/*.md` + `references/18-history-derived-governance.md` + `references/10-version-evidence.md` | `skill-creator`、`references/visual/reference-entries.md`、`skills/skills.manifest.yaml` | 输出《完整Skill审计矩阵》和《历史总结新规则对齐矩阵》：入口触发、流程阻断、部门职责、QA证据、经验回写、元数据、验证脚本、旧summary/evidence状态逐项检查；每项修订必须绑定触发、证据、阻断、责任和文件路径 |
...
- 完整Skill审计任务：追加《用户最新要求锁定表》《历史对话需求整合表》《完整Skill审计矩阵》《历史总结新规则对齐矩阵》《规则闭环映射表》《修订影响面清单》和验证记录；必须证明本次未创建或修改交付产物目录。
